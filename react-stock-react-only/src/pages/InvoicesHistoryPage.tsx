...
import {
  fetchAllInvoicesWithDetails,
  downloadInvoiceXml,
  type InvoiceDetails,
} from '../utils/invoiceApi'
import type { KSefStatus } from '../utils/types'
import { parseFA3Invoice, type ParsedInvoice } from '../utils/xmlParser'
...
const InvoicesHistoryPage = () => {
  ...
  const handleDownloadXml = async (invoice: InvoiceDetails) => {
    try {
      const xml = await downloadInvoiceXml(invoice.referenceNumber)
      const safeNumber = (invoice.invoiceNumber || invoice.referenceNumber).replace(
        /\//g,
        '-',
      )
      const fileName = invoice.ksefNumber
        ? `FV_${safeNumber}_KSeF_${invoice.ksefNumber}.xml`
        : `FV_${safeNumber}.xml`
      ...
      alert(`Nie udało się pobrać pliku XML: ${err instanceof Error ? err.message : err}`)
    }
  }

  const handleGeneratePdf = async (invoice: InvoiceDetails) => {
    try {
      const xml = await downloadInvoiceXml(invoice.referenceNumber)
      const parsed = parseFA3Invoice(xml, invoice.shop)
      setPdfPreview({
        ...parsed,
        ksefNumber: invoice.ksefNumber,
        verificationUrl: invoice.qrCode?.data,
      })
    } catch (err) {
      console.error('Error generating PDF:', err)
      alert(`Nie udało się wygenerować PDF: ${err instanceof Error ? err.message : err}`)
    }
  }
  ...
  return (
    <StyledMain>
      ...
      {pdfPreview && (
        <OffScreenRender>
          <InvoiceLayout
            invoiceData={pdfPreview.invoiceData}
            productsData={pdfPreview.productsData}
            extraProduct={[]}
            onGenerateXML={async () => {}}
            ksefNumber={pdfPreview.ksefNumber}
            verificationUrl={pdfPreview.verificationUrl}
            sellerNip="8442120248"
            autoDownloadPdf
            onPdfDownloaded={() => setPdfPreview(null)}
          />
        </OffScreenRender>
      )}
    </StyledMain>
...
const OffScreenRender = styled.div`
  position: fixed;
  left: -9999px;
  top: 0;
  z-index: -1;
