...
import { validateInvoiceData } from '../src/utils/invoiceValidator.js'
import type { InvoiceData, ValidationResult } from '../src/utils/types.js'
  ...
  try {
    ...
    if (!validation.isValid) {
      ...
      return res.status(400).json({
        success: false,
        error: 'Błędy walidacji faktury',
        validationErrors: validation.errors,
        validationWarnings: validation.warnings,
      })
    }

    // Pokaż ostrzeżenia w konsoli (ale nie blokuj)
    ...
    if (contentType && contentType.includes('application/json')) {
      const ksefData = (await ksefResponse.json()) as {
        outcome?: { referenceNumber?: string }
...
app.get('/api/ksef-invoice/:referenceNumber', async (req: Request, res: Response) => {
  try {
    const { referenceNumber } = req.params
    console.log(`📥 Fetching invoice details for: ${referenceNumber}`)

    const ksefResponse = await fetch(`${KSEF_API_URL}/${referenceNumber}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (contentType && contentType.includes('application/json')) {
      const invoiceDetails = (await ksefResponse.json()) as {
        [key: string]: unknown
      }
...
app.listen(PORT, () => {
  ...
  console.log(`   Validate invoice: http://localhost:${PORT}/api/validate-invoice`)
