...
export function validateInvoiceData(invoiceData: InvoiceData): ValidationResult {
  ...
  checkedProducts.forEach((product, index) => {
    ...
    // PKWiU/Code validation (max 50 chars)
    if (product.code && product.code.length > 50) {
      errors.push(
        `${productLabel}: Kod PKWiU za długi (${product.code.length}/50 znaków)`,
      )
    }
    ...
    calculatedTotalGross += product.totalGross
    if (!calculatedVatSummary[product.vat]) {
      calculatedVatSummary[product.vat] = { net: 0, vat: 0, gross: 0 }
    }
    calculatedVatSummary[product.vat].net += product.totalNet
    calculatedVatSummary[product.vat].vat += product.totalGross - product.totalNet
    calculatedVatSummary[product.vat].gross += product.totalGross
  })
