export function generateFA3Invoice(invoiceData) {
  ...
  let totalGrossAll = 0
  const vatSummary = { 8: { net: 0, vat: 0 }, 23: { net: 0, vat: 0 } }

  checkedProducts.forEach((product) => {
    totalGrossAll += product.totalGross
    const rate = product.vat.toString()
    if (vatSummary[rate]) {
      vatSummary[rate].net += product.totalNet
      vatSummary[rate].vat += product.totalGross - product.totalNet
    }
  })

  // 2. Generate VAT Summary Tags (Aligned with P_13_x schema)
  let vatFieldsXml = ''
  if (vatSummary['8'].net > 0) {
    vatFieldsXml += `\n    <P_13_2>${formatAmount(vatSummary['8'].net)}</P_13_2>\n    <P_14_2>${formatAmount(vatSummary['8'].vat)}</P_14_2>`
  }
  if (vatSummary['23'].net > 0) {
    vatFieldsXml += `\n    <P_13_1>${formatAmount(vatSummary['23'].net)}</P_13_1>\n    <P_14_1>${formatAmount(vatSummary['23'].vat)}</P_14_1>`
  }
  ...
  checkedProducts.forEach((product, index) => {
    ...
    <FaWiersz>
      ...
      <P_11A>${Math.round(product.totalGross)}</P_11A>
...
<Faktura xmlns="http://crd.gov.pl/wzor/2025/06/25/13775/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <Fa>
    ...
    <P_6>${formatDate(invoiceData.endSaleDate || invoiceData.invoiceDate)}</P_6>${vatFieldsXml}
