...
export function generateFA3Invoice(invoiceData) {
  ...
  checkedProducts.forEach((product, index) => {
    ...
    <FaWiersz>
      ...
      ${
        product.pkwiu
          ? `
      <PKWiU>${escapeXml(product.pkwiu)}</PKWiU>`
          : ''
      }
  ...
  // 4. Generate PKWiU codes as DodatkowyOpis (after FaWiersz)
  let pkwiuXml = ''
  checkedProducts.forEach((product, index) => {
    if (product.pkwiu) {
      pkwiuXml += `
    <DodatkowyOpis>
      <NrWiersza>${index + 1}</NrWiersza>
      <Klucz>PKWiU</Klucz>
      <Wartosc>${escapeXml(product.pkwiu)}</Wartosc>
    </DodatkowyOpis>`
    }
  })

  // 5. Podmiot3 Alignment (Sequence: DaneIdentyfikacyjne -> Adres -> Rola)
  let podmiot3Xml = ''
...
<Faktura xmlns="http://crd.gov.pl/wzor/2025/06/25/13775/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema">
  ...
  <Fa>
    ...
    <DodatkowyOpis>
      <Klucz>Okres dostawy</Klucz>
      <Wartosc>${formatDate(invoiceData.startDate)} - ${formatDate(invoiceData.endDate)}</Wartosc>
    </DodatkowyOpis>
    ${pkwiuXml}
    ${productsXml}
    <Platnosc>
