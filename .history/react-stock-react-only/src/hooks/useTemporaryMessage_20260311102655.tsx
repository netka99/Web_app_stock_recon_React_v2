...
const useTemporaryMessage = (
  initialMessage = '',
): [string, (newMessage: string, duration?: number | null) => void] => {
  const [message, setMessage] = useState<string>(initialMessage)
  const timeoutRef = useRef<NodeJS.Timeout | null>(null) // Keeps track of the active timeout

  const showMessage = (newMessage: string, duration: number | null = 3000): void => {
    ...
    // If duration is null, message stays until manually closed
    if (duration !== null) {
