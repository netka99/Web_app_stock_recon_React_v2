...
const isValidProduct = (product: string): product is ProductName => {
  return ['Kartacze', 'Babka', 'Kiszka'].includes(product)
}
...
const SummaryPerProduct: React.FC<SummaryPerProductProps> = ({
  ...
  const handleUpdateLocalSaleData: Dispatch<SetStateAction<SaleData[]>> = (
    newLocalSale,
  ) => {
    setLocalSaleData(newLocalSale)
  }

  const handleUpdateLocalReturnData: Dispatch<SetStateAction<ReturnData[]>> = (
    newLocalReturn,
  ) => {
          ...
          <div key={`${shop}-${index}`} className="summary-total">
            <div className="summary-header">
              ...
              <div className="summary-total-quantity">
                {isValidProduct(productSelected)
                  ? `${totals(shop).toFixed(2)} ${units[productSelected as ProductName]}`
                  : 'Invalid product'}
              </div>
              <div className="summary-total-prize">
                {isValidProduct(productSelected)
                  ? `${((totals(shop) * settingsData.prices[productSelected]) / 100).toFixed(2)} zł`
                  : 'Invalid product'}
              </div>
