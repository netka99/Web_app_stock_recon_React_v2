...
import { getAllInvoices, deleteInvoice, getInvoicesByStatus } from '../utils/invoiceStorage'
...
const InvoicesHistoryPage = () => {
  ...
  const loadInvoices = () => {
    if (filterStatus === 'all') {
      setInvoices(getAllInvoices().sort((a, b) =>
        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      ))
    } else {
      setInvoices(getInvoicesByStatus(filterStatus).sort((a, b) =>
        new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
      ))
    }
  }
  ...
  const stats = {
    ...
    error: invoices.filter(i => i.ksefStatus === 'error' || i.ksefStatus === 'rejected').length,
  }
                ...
                <InvoiceCard key={invoice.id}>
                  ...
                  <InvoiceDetails>
                    ...
                    {invoice.ksefReferenceNumber && (
                      <DetailRow>
                        <DetailLabel>Nr referencyjny KSeF:</DetailLabel>
                        <DetailValue className="mono">
                          {invoice.ksefReferenceNumber}
                        </DetailValue>
                      </DetailRow>
                    )}
