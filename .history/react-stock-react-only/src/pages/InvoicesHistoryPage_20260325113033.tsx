...
const InvoicesHistoryPage = () => {
  ...
  const getKSefStatus = (invoice: InvoiceDetails): KSefStatus => {
    if (invoice.ksef_number) return 'accepted'
    if (invoice.status_code && invoice.status_code >= 400) return 'error'
    if (invoice.status_code && invoice.status_code === 200) return 'sent'
    return 'draft'
  }
        ...
        {loading ? (
          ...
          <InvoicesList>
            {filteredInvoices.map((invoice) => {
              ...
              return (
                <InvoiceCard key={invoice.reference_number}>
                  <InvoiceHeader>
                    <InvoiceNumber>
                      {invoice.invoice_number ||
                        `Ref: ${invoice.reference_number.split('-').pop()}`}
                    </InvoiceNumber>
                  ...
                  <InvoiceDetails>
                    ...
                    {invoice.buyer_nip && (
                      <DetailRow>
                        ...
                        <DetailValue>{invoice.buyer_nip}</DetailValue>
                      </DetailRow>
                    )}
                    <DetailRow>
                      <DetailLabel>Data faktury:</DetailLabel>
                      <DetailValue>{invoice.issue_date || '-'}</DetailValue>
                    </DetailRow>
                    <DetailRow>
                      <DetailLabel>Wartość brutto:</DetailLabel>
                      <DetailValue>
                        {(invoice.total_gross_cents / 100).toFixed(2)} zł
                      </DetailValue>
                    </DetailRow>
                    {invoice.ksef_number && (
                      <DetailRow>
                        <DetailLabel>Numer faktury KSeF:</DetailLabel>
                        <DetailValue className="mono">{invoice.ksef_number}</DetailValue>
                      </DetailRow>
                    )}
                    {invoice.status_description &&
                      invoice.status_code &&
                      invoice.status_code >= 400 && (
                        <DetailRow className="error">
                          <DetailLabel>Błąd:</DetailLabel>
                          <DetailValue>
                            {invoice.status_description} (kod: {invoice.status_code})
                          </DetailValue>
                        </DetailRow>
                      )}
                    <DetailRow className="meta">
                      <DetailLabel>Utworzono:</DetailLabel>
                      <DetailValue>{formatDate(invoice.created_at)}</DetailValue>
                    </DetailRow>
                    <DetailRow className="meta">
