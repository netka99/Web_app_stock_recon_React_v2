...
const InvoicePage = () => {
  ...
  const generateKSeFInvoice = useCallback(async () => {
    try {
      showMessage('⏳ Wysyłanie faktury do KSeF...', 3000)
      ...
      const validationResponse = await fetch(
        'http://localhost:5000/api/validate-invoice',
        {
          method: 'POST',
      ...
      if (validationResult.warnings && validationResult.warnings.length > 0) {
        const warningsText = validationResult.warnings.join('\n• ')
        console.warn('⚠️ Ostrzeżenia walidacji:', validationResult.warnings)
      ...
      if (!validationResult.isValid) {
        ...
        return // Stop here - don't send to KSeF
      }

      // Send to KSeF if validation passed =====
      showMessage('✅ Walidacja OK. Wysyłanie faktury do KSeF...', 3000)

      const response = await fetch('http://localhost:5000/api/send-ksef-invoice', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(invoicePayload),
      })
      ...
      if (response.ok && data.success) {
        ...
        // Add validation warnings to success message if any
        if (data.validationWarnings && data.validationWarnings.length > 0) {
          const warningsText = data.validationWarnings.join('\n• ')
          successMessage += `\n\n⚠️ Ostrzeżenia:\n• ${warningsText}`
        }
        ...
        if (data.validationErrors && data.validationErrors.length > 0) {
          const errorsText = data.validationErrors.join('\n• ')
          showMessage(
            `❌ Błędy walidacji faktury:\n\n• ${errorsText}\n\nPopraw dane i spróbuj ponownie.`,
            15000,
          )
...
`
Główne zmiany w media query:
Usunąłem left: 5% i transform: none
Dodałem width: 90% zamiast min/max width
Zachowałem left: 50% i transform: translateX(-50%) - to wycentruje popup
min-width: unset i max-width: unset - usuwamy limity szerokości na mobile
Teraz popup będzie wyśrodkowany i zajmie 90% szerokości ekranu na telefonie/tablecie, z równymi marginesami po obu stronach! 📱✅
