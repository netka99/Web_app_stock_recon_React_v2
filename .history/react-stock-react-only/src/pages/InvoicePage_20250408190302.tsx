...
const InvoicePage = () => {
  ...
  const fetchDataByAPI = useCallback(
    async (url: string, setDatafromAPI: React.Dispatch<React.SetStateAction<any>>) => {
      try {
  ...
  const summary = useCallback(
    (
      data: SaleValue[],
      shop: string,
      stateVar: React.Dispatch<React.SetStateAction<Record<string, number>>>,
    ) => {
      const filteredByShop = data?.filter((d) => d.shop === shop)

      const productSum = filteredByShop?.reduce((acc: Record<string, number>, curr) => {
  ...
  const totalsPerProduct = useCallback(() => {
    const safeSummarySale: Record<string, number> = summarySale || {}
    const safeSummaryReturns: Record<string, number> = summaryReturns || {}
    ...
    const updateProductQuantityTotals = productsData.map((product) => {
      const key = product.product as string
      const quantity = totals[key] ?? 0
