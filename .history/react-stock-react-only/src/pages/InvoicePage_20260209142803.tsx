...
const InvoicePage = () => {
  ...
  const generateKSeFInvoice = useCallback(async () => {
    try {
      ...
      a.href = url
      a.download = `invoice-${invoiceData.invoiceNumber.replace(/\//g, '-')}.xml`
      a.click()
      window.URL.revokeObjectURL(url)

      showMessage('XML został wygenerowany!', 6000)
    } catch (error) {
      showMessage('Błąd podczas generowania XML', 6000)
    }
      ...
      <Container>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setInvoiceData((prev) => ({
                        ...
                        buyer_nip: settings.buyer_nip?.[selectedShop] || '',
                      }))
                    }}
                  >
        ...
        {((summarySale && Object.keys(summarySale).length > 0) ||
          ...
          <div className="generateButton">
            ...
            <button
              className={isInvoiceVisible ? 'hide' : ''}
              onClick={generateKSeFInvoice}
            >
              Generuj XML dla KSeF{' '}
            </button>
          </div>
        )}
        {isInvoiceVisible && (
