...
import { updateInvoiceStatus as updateBackendInvoiceStatus } from '../utils/invoiceApi'
import { authenticateWithKsef } from '../utils/invoiceApi'
...
const InvoicePage = () => {
      ...
      <ModalContent>
        ...
        <ModalBody>
          ...
          >
            ...
            />
            <ModalActions>
              <CancelButton type="button" onClick={() => setShowPassphraseModal(false)}>
                Anuluj
              </CancelButton>
  ...
  const generateKSeFInvoice = useCallback(async () => {
    ...
    try {
      ...
      // Show passphrase modal instead of sending immediately
      setShowPassphraseModal(true)
      return // Stop here, modal will trigger handleAuthAndSend
      ...
      // Calculate total gross in cents (grosz)
      ...
      if (response.ok && referenceNumber) {
        ...
        try {
          ...
          if (detailsResponse.ok && detailsData.ksefNumber) {
            ...
            // Update backend database with KSeF invoice details
            try {
              await updateBackendInvoiceStatus(referenceNumber, {
                ksef_number: detailsData.ksefNumber,
                invoice_number: detailsData.invoiceNumber,
                status_code: detailsData.status?.code,
                status_description: detailsData.status?.description,
              })
              console.log('✅ Backend database updated with KSeF details')
            } catch (patchError) {
              console.error('⚠️ Failed to update backend database:', patchError)
              // Don't fail the whole process if PATCH fails
            }
  ...
  const handleAuthAndSend = async () => {
    ...
    try {
      ...
      showMessage('🔐 Uwierzytelnianie z KSeF...', 2000)
      ...
      // Step 2: Continue with invoice sending (copy from old code)
      ...
      // Send invoice WITHOUT passphrase
      const response = await fetch(ksefApiUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      if (response.ok && referenceNumber) {
        ...
        // Continue with fetching invoice details (keep existing code from line 598-700)
        // ... rest of the flow
      } else {
      ...
      console.error('❌ Error:', error)
      showMessage(`❌ Błąd: ${error.message}`, 10000)
    } finally {
