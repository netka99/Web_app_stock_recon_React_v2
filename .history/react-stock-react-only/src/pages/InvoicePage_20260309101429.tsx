  ...
  const generateKSeFInvoice = useCallback(async () => {
    try {
      ...
      if (response.ok && data.success) {
        ...
        // Fetch additional invoice details using reference number
        if (data.referenceNumber) {
          showMessage('✅ Faktura wysłana! Pobieranie szczegółów...', 3000)

          try {
            const detailsResponse = await fetch(
              `http://localhost:5000/api/ksef-invoice/${data.referenceNumber}`,
              {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' },
              },
            ...
            if (detailsResponse.ok && detailsData.success) {
              console.log('✅ Invoice details:', detailsData.data)

              let successMessage = `✅ Faktura wysłana do KSeF!\n\nNumer referencyjny:\n${data.referenceNumber}\n\nSzczegóły faktury pobrane pomyślnie!`
              ...
              // You can now use detailsData.data for additional processing
            } else {
              console.warn('⚠️ Could not fetch invoice details:', detailsData)
            ...
            const successMessage = `✅ Faktura wysłana do KSeF!\n\nNumer referencyjny:\n${data.referenceNumber}\n\n⚠️ Błąd podczas pobierania szczegółów`

            showMessage(successMessage, 15000)
          }
        } else {
          showMessage(
            `✅ Faktura wysłana do KSeF!\n\n⚠️ Brak numeru referencyjnego w odpowiedzi`,
            15000,
          )
        }
      } else {
