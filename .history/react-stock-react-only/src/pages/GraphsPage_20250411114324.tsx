...
import { productsData, ProductName } from '../utils/productDetails'
...
const GraphsPage = () => {
  ...
  const [saleByProduct, setSaleByProduct] = useState<ProductName>('Kartacze')
  ...
  const summary = (date: string, data: SaleValue[], shop: string) => {
  ...
  const sumPerDay = () => {
    const updatedDatesforSale = dates.map((d) => ({
      ...d,
      sprzedaż: summary(d.name, sale, selectedShop) || 0,
      zwroty: summary(d.name, returns, selectedShop) || 0,
    }))
  ...
  const searchByDate = async () => {
    ...
    try {
      await fetchDataByAPI(VITE_APP_SETTINGS_API, setSettings)
      await fetchDataByAPI(urlSales, (data: SaleValue[]) => {
        setSale(data)
      })
      await fetchDataByAPI(urlReturns, (data: SaleValue[]) => {
        setReturns(data)
      })
