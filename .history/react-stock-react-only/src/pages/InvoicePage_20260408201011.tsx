...
const InvoicePage = () => {
  ...
  const generateKSeFInvoice = useCallback(async () => {
    ...
    try {
      ...
      localStorage.setItem(`invoiceXML_${savedInvoiceId}`, invoiceXML)

      // STEP 3: Check if KSeF session is active
      showMessage('🔐 Sprawdzanie sesji KSeF...', 1500)
      const activeSessions = await checkKsefSession()
      ...
      if (activeSessions.length === 0) {
        ...
        await handleAuthAndSend(false) // false = skip authentication
      }
    } catch (error) {
      if (savedInvoiceId) {
  ...
  const handleAuthAndSend = async (skipAuth: boolean = false) => {
    ...
    try {
      if (!skipAuth) {
        ...
        try {
          ...
          if (error instanceof Error && error.message.includes('w trakcie')) {
            // 422 error - authentication pending, retry after delay
      ...
      // ✅ REUSE XML from generateKSeFInvoice instead of regenerating
      const invoiceXML = localStorage.getItem(`invoiceXML_${savedInvoiceId}`)
      if (!invoiceXML) {
        throw new Error('Invoice XML not found in cache')
      }
      const invoiceBase64 = btoa(unescape(encodeURIComponent(invoiceXML)))

      // ✅ REUSE totals from generateKSeFInvoice instead of recalculating
      const totals = JSON.parse(
        localStorage.getItem(`invoiceTotals_${savedInvoiceId}`) || '{}',
      )
      const totalGrossCents = totals.totalGrossCents
      if (!totalGrossCents) {
        throw new Error('Invoice totals not found in cache')
      }

      localStorage.setItem(
        `invoiceTotals_${savedInvoiceId}`,
        JSON.stringify({
          totalGrossCents,
        }),
      )
