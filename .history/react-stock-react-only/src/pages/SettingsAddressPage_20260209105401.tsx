...
const SettingsAddressPage = () => {
  interface SettingsData {
    ...
    shops: string[]
    buyer_nip: Record<string, string>
  }
  ...
  const [buyerNIPs, setBuyerNIPs] = useState<Record<string, string>>({})
  ...
  const changeNIP = (shop: string, value: string): void => {
    setBuyerNIPs({
      ...buyerNIPs,
      [shop]: value,
    })
              ...
              return (
                <div key={`${shop}_${index}`} className="address-container">
                  ...
                  <div className="shop-address">
                    {editingShop === shop ? (
                      <textarea
                        value={buyerNIPs[shop] || ''}
                        onChange={(e) => changeNIP(shop, e.target.value)}
                        rows={1}
                        placeholder="Wprowadź NIP"
                      />
                    ) : buyerNIPs[shop] ? (
                      <div>
                        <div className="address">{buyerNIPs[shop]}</div>
                      </div>
                    ) : (
                      <div>Brak NIP</div>
                    )}
