...
export function generateFA3Invoice(invoiceData) {
  ...
  // JST i GV - ustawiamy na podstawie Podmiot3 i jego roli
  // const hasPodmiot3JST =
  //   invoiceData.buyer_data.hasPodmiot3 &&
  //   invoiceData.buyer_data.podmiot3 &&
  //   invoiceData.buyer_data.podmiot3.rola === '8'
  // const hasPodmiot3GV =
  //   invoiceData.buyer_data.hasPodmiot3 &&
  //   invoiceData.buyer_data.podmiot3 &&
  //   invoiceData.buyer_data.podmiot3.rola === '10'

  // podmiot2.ele('JST').txt(hasPodmiot3JST ? '1' : '2') // 1=tak, 2=nie
  // podmiot2.ele('GV').txt(hasPodmiot3GV ? '1' : '2') // 1=tak, 2=nie
  ...
  if (invoiceData.buyer_data.hasPodmiot3 && invoiceData.buyer_data.podmiot3) {
    ...
    // BrakID - wymagane dla Podmiot3 bez NIP/VAT
    daneOdbiorcy.ele('BrakID').txt('1')
    // IdWew - identyfikator wewnętrzny klienta
  ...
  fa.ele('FP').txt('2') // Faktura wystawiona w cenach: 1=Netto, 2=Brutto
  ...
  invoiceData.products
    ...
    .forEach((product, index) => {
      ...
      wiersz.ele('P_9B').txt(product.grossPrice.toFixed(2)) // Cena jednostkowa BRUTTO (nie P_9A)
      wiersz.ele('P_11A').txt(product.totalGross.toFixed(2)) // Wartość BRUTTO (nie P_11)
