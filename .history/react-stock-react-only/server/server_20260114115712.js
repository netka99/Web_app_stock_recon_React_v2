...
app.post('/api/ksef/login', async (req, res) => {
  try {
    ...
    console.log('Encrypted Token (Base64 Preview):', encryptToken)
...
app.get('/api/ksef/invoices', async (req, res) => {
  ...
  try {
    const response = await axios.post(
      `${KSEF_URL}/online/Query/Invoice/Sync`,
      {
        queryCriteria: {
          subjectType: 'SUBJECT',
          acquisitionTimestampThresholdFrom: '2024-01-01T00:00:00Z',
          acquisitionTimestampThresholdTo: '2026-01-14T23:59:59Z',
        },
        pageSize: 10,
        pageOffset: 0,
      },
...
app.post('/api/ksef/invoice', async (req, res) => {
  const sessionToken = req.headers['session-token']
  const { ksefReferenceNumber } = req.body

  if (!sessionToken || !ksefReferenceNumber) {
    return res.status(400).json({
      error: 'Missing SessionToken or ksefReferenceNumber',
    })
  }

  try {
    const response = await axios.post(
      `${KSEF_URL}/online/Invoice/Get`,
      {
        ksefReferenceNumber,
      },
      {
    ...
    res.set({
      ...
      'Content-Disposition': `attachment; filename="${ksefReferenceNumber}.xml"`,
    })

    res.send(response.data)
  } catch (error) {
    console.error('Invoice Get Error:', error.response?.data || error.message)
    res.status(error.response?.status || 500).json({
      error: 'Invoice download failed',
      details: error.response?.data,
    })
  }
})
