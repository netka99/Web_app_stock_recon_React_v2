...
export function generateFA3Invoice(invoiceData) {
  ...
  fa.ele('P_1M').txt('Warszawa') // City of issuance fa.ele('P_1M').txt(invoiceData.city)
  fa.ele('P_2').txt(invoiceData.invoiceNumber) // Invoice number
  // Jeśli jest ZAKRES dat (różne daty) - używamy OkresFa (BEZ P_6)
  if (
    ...
    // Jeśli jest POJEDYNCZA data (lub brak dat) - używamy P_6 (BEZ OkresFa)
    fa.ele('P_6').txt(invoiceData.endSaleDate || invoiceData.invoiceDate)
  }
  ...
  // Dynamiczne dodawanie sum dla każdej stawki VAT - POSORTOWANE ROSNĄCO
  const sortedVatRates = Object.keys(totals.vatGroups)
    .map(Number)
    .sort((a, b) => a - b) // Sortowanie rosnąco: 0, 3, 5, 7, 8, 23
  ...
  // ===== PAYMENT INFO (Platnosc) =====
  const platnosc = fa.ele('Platnosc')

  // Termin płatności
  const terminPlatnosci = platnosc.ele('TerminPlatnosci')
  terminPlatnosci.ele('Termin').txt(invoiceData.paymentDate)

  // Forma płatności - mapowanie z tekstowych wartości na kody KSeF
  const formaPlMap = {
    Gotówka: '1',
    Przelew: '6',
  }
  const formaPlCode = formaPlMap[invoiceData.paymentType] || '6' // Domyślnie przelew
  platnosc.ele('FormaPlatnosci').txt(formaPlCode)

  // Rachunek bankowy - tylko dla przelewu
  if (invoiceData.paymentType === 'Przelew') {
    const rachunekBankowy = platnosc.ele('RachunekBankowy')
    rachunekBankowy.ele('NrRB').txt('TU_WSTAW_NUMER_KONTA') // Zastąp swoim numerem konta
    rachunekBankowy.ele('SWIFT').txt('BPKOPLPW') // Zastąp swoim kodem SWIFT
  }
