...
export function generateFA3Invoice(invoiceData) {
  ...
  invoiceData.products
    ...
    .forEach((product, index) => {
      ...
      wiersz.ele('P_11A').txt((product.totalNet * (product.vat / 100)).toFixed(2)) // ← ADD THIS LINE
      wiersz.ele('P_12').txt(product.vat)
      //   wiersz.ele('P_12_X').txt('1')
    })
  ...
  adnotacje.ele('P_19').txt('2') // Exempt from VAT
  adnotacje.ele('P_22').txt('2') // New means of transport
  adnotacje.ele('P_23').txt('2') // Simplification procedures
  adnotacje.ele('P_PMarzy').txt('2') // Margin procedures
