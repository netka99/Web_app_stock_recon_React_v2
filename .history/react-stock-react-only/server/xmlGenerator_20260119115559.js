...
export function generateFA3Invoice(invoiceData) {
  ...
  invoiceData.products
    ...
    .forEach((product, index) => {
      ...
      wiersz.ele('P_7').txt(product.productName) // Product name
      wiersz.ele('P_8A').txt(product.units) // Unit (szt., kg)
      wiersz.ele('P_8B').txt(product.quantity) // Quantity
      wiersz.ele('P_9A').txt(product.netPrice) // Net price
      wiersz.ele('P_11').txt(product.totalNet) // Total net
      wiersz.ele('P_12').txt(product.vat) // VAT rate
    })
}
