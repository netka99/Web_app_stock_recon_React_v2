...
export function generateFA3Invoice(invoiceData) {
  ...
  invoiceData.products
    ...
    .forEach((product, index) => {
      ...
      // TODO 8: Fill in product details
      wiersz.ele('P_7').txt(product.???);  // Product name
      wiersz.ele('P_8A').txt(product.???); // Unit (szt., kg)
      wiersz.ele('P_8B').txt(product.???); // Quantity
      wiersz.ele('P_9A').txt(product.???); // Net price
      wiersz.ele('P_11').txt(product.???); // Total net
      wiersz.ele('P_12').txt(product.???); // VAT rate
    });
}
