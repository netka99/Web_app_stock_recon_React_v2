...
app.get('/api/ksef/session/status', async (req, res) => {
  const sessionToken = req.headers['session-token']
  // We use the reference number from the login response
  const sessionRef = req.query.referenceNumber

  try {
    const response = await axios.get(`${KSEF_URL}/online/Session/Status/${sessionRef}`, {
      headers: { SessionToken: sessionToken },
    })
    res.json(response.data)
  } catch (error) {
    res
      ...
      .json(error.response?.data || { error: 'Session check failed' })
  }
})
